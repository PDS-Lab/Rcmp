...
class UDPClient {
   public:
    ...
    size_t send(const std::string rem_hostname, uint16_t rem_port, const T &msg) {
        ...
        if (results.size() == 0) {
            DLOG_ERROR("eRPC: Failed to resolve %s, asio error = %s.\n", rem_hostname.c_str(),
                       error.message().c_str());
            return SIZE_MAX;
        }
        ...
        for (const auto &endpoint_iter : results) {
            ...
            try {
                const size_t ret = socket_->send_to(asio::buffer(&msg, sizeof(T)), endpoint_iter);
                if (enable_recording_flag_) sent_vec_.push_back(msg);
                return ret;
            } catch (const asio::system_error &e) {
                DLOG_ERROR("eRPC: asio send_to() failed to %s, error: %s\n", rem_hostname.c_str(),
                           e.what());
                return SIZE_MAX;
            }
        }
